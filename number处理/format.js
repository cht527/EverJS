const FormatSet = {
  ...
  isNumberInPercent100: (value, isZeroValid = true) => {
    ...
    return (
      isValid && /^(100(\.0{1,9})?|(([1-9]){1}[0-9]?|0{1})((\.)([0-9]){1,9})?)$/.test(`${value}`)
    );
  },
  /**
   * 
  ...
  getRandomNumber: (min, max, decimalPlaces = 0) => {
    ...
    return Number(
      ((Number(randomData.toString()) / 256) * (max - min + 1) + min).toFixed(decimalPlaces),
    );
  },
  /**
  ...
  convertNumberToShortFormAndUnit: (
    ...
    const shortForm = FormatSet.getStringOfFloat(isNegative ? absShortForm * -1 : absShortForm);
  ...
  getFormatNumberStr: (number, options) => {
    if (number <= Number.MAX_SAFE_INTEGER && number >= Number.MIN_SAFE_INTEGER) {
      ...
      // 再判断是否去掉小数点后面的 0
      const numberToFormat = isRemoveSuffixZero ? FormatSet.removeSuffixZero(formattedNumber) : formattedNumber;
      // 最后格式化千分位（包括小数）
   ...
   * 
   * @param {*} number number
   * @param {*} options precision?: number; isRemoveSuffixZero?: boolean; edgeSmallNumber?: number 
   * @returns 
   */
  getPreciseFormatNumber:(number, options) => {
    const { precision = 2, isRemoveSuffixZero = true, edgeSmallNumber = 0.01 } = options ?? {};
